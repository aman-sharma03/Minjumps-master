onload = function () {
    const unit = 14;
    const move_step = unit * 12;
    const fly_step = unit * 18;
    let platforms_array = [];
    let moves = [];
    let pos = 0;
    let firstTime = true;
    ...
    const BootState = {
        init : function () {
            game.stage.backgroundColor = '#a5c3cf';
    ...
    function createGame() {
        ...
        sun = game.add.sprite(10*unit, ground_height - 20*unit, 'sun');
        ...
        for (i = 0; i < 30; i++) {
            ...
            game.add.text(pos + 10, ground_height-30, platforms_array[i], result_font);
            if(i===29){
                platform.scale.setTo(2,1);
            }
        }

        const ground = game.add.tileSprite(0,ground_height+4, game_length, 4, 'wave');
        game.physics.arcade.enable(ground);
        ground.body.immovable = true;
        game.add.tileSprite(0,ground_height+7, game_length, game.height, 'sea');

        flag = game.add.sprite(1785, ground_height-168, 'flag');
        flag.animations.add('celebrate');

        player = game.add.sprite(20, ground_height - 17, 'mario');
        ...
        if (player.body.enable) {
            if (onGround) {
                ...
                if(pos<moves.length) {
                    ...
                    player.body.gravity.y = fly_step*2;
                }
            }
